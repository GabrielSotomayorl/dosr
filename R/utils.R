...
truncate_sheet_name <- function(name) {
  if (nchar(name) > 31) return(stringr::str_trunc(name, 31, side = "right", ellipsis = ""))
  else return(name)
}
unique_cols <- function(df) { names(df) <- make.unique(names(df)); df }
hdrStyle  <- createStyle(fgFill = "#D9D9D9", textDecoration = "bold", halign = "center", valign = "center", border = "TopBottomLeftRight", borderColour = "black")
bodyStyle <- createStyle(border = "TopBottomLeftRight", borderColour = "black", valign = "center")


#' Write a data frame to an Excel sheet with clean and safe styling.
#' This is the centralized function to write all tables.
#' @noRd
write_clean_table <- function(wb, sheet, x, startRow = 1, startCol = 1, na_string = "") {
  ...
  openxlsx::writeData(wb, sheet, x,
                      ...
                      headerStyle = hdrStyle, na.string = na_string)

  rows_body <- (startRow + 1):(startRow + nrow(x))
  cols_all  <- startCol:(startCol + ncol(x) - 1)

  openxlsx::addStyle(wb, sheet, bodyStyle,
